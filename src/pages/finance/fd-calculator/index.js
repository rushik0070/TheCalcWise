...
export default function Fd() {
    ...
    const [mode, setMode] = useState('without-inflation'); // 'lumpsum' or 'sip'
    const [investment, setInvestment] = useState(100000); // monthly for SIP
    ...
    const n = 4;
    let totalValue = 0;
    let maturity = investment * Math.pow(1 + rate / (100 * n), n * time);
    let estimatedReturns = maturity - investment;
    let inflationAdjustedMaturity = maturity;
    let investedAmount = inflationAdjustedMaturity - estimatedReturns;

    if (mode === 'with-inflation') {

        inflationAdjustedMaturity = maturity / Math.pow(1 + inflationRate / 100, time);
        estimatedReturns = inflationAdjustedMaturity - investment;
    }
    totalValue = inflationAdjustedMaturity
    const chartData = {
            ...
            <div className='row mt-5'>
                ...
                <div className='col-md-7'>
                    <div className="calc-container">
                        ...
                        <div className='row'>
                            <div className='col-6'>
                                <h4 className="fw-bold mb-4">FD Calculator
                                </h4>
                            ...
                            <div className='col-6 text-end'>
                                <div className="btn-group border">
                                    <button
                                        className={`btn btn-${mode === 'without-inflation' ? 'success' : 'light'}`}
                                        onClick={() => setMode('without-inflation')}
                                    >
                                        Without Inflation
                                    </button>
                                    <button
                                        className={`btn btn-${mode === 'with-inflation' ? 'success' : 'light'}`}
                                        onClick={() => setMode('with-inflation')}
                                    >
                                        With Inflation
                                    </button>
                                </div>
                                    ...
                                    <div className="col-md-6">
...

                                        {
                                            mode === 'with-inflation' && (
                                                ...
                                                <div className="mb-3">
                                                    <label className="form-label fw-bold">Set Inflation Rate (p.a)</label>
